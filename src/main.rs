...
fn test_parse_pair() {
    ...
    assert_eq!(parse_pair::<i32>("0.5x2.5", 'x'), None);
}
...
/// iteration limit before being able to prove that `c` is not a member),
/// return `None`.
...
fn render(
    ...
    for row in 0..bounds.1 {
        for column in 0..bounds.0 {
            ...
            pixels[row * bounds.0 + column] = match complex_square_add_loop(point, 255) {
                None => 0,
